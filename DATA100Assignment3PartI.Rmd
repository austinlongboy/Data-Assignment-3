...
at_cyclone <- str_c(cyclone_data_address, at_cyclone_filename, sep = "") |>
  ...
  mutate(
    ...
    # make "-999" and "-99" NAs (once)
    across(everything(), ~ .x |> na_if("-999") |> na_if("-99")),
...
aware_fixed_country_match <- aware_fixed_country |>
  mutate(
    country = case_when(
      ...
      country == "South Korea" ~ "Korea, Dem. People's Rep.",
      country == "Sub-Saharan Africa" ~ "Central African Republic",
...
covid_2020_match_country <- covid_2020 |>
  mutate(
    country = case_when(
      ...
      country == "Saint Martin (French part)" ~ "St. Martin (French part)",
      country == "Saint Vincent and the Grenadines" ~ "St. Vincent and the Grenadines",
      country == "Somalia" ~ "Somalia, Fed. Rep.",
...
covid_countries <- covid_2020_match_country |> pull(country)
setdiff(covid_countries, pop_countries) |> sort()
```
...
aware_covid <- aware_pop |>
  inner_join(
    covid_2020_match_country |> 
      select(country, total_cases, continent),
    by = "country"
  ) |>
